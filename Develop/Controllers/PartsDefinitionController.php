...
class PartsDefinitionController extends \Develop\Utils\BaseController {
    ...
    public function editPartsAction() {
        ...
        \Develop\Utils\Screen::updateAreaE(self::VIEW_REGIST, [
            ...
            'search_area'          => $targetParts['search_area'] ?? 0,
            'detail_area'          => $targetParts['detail_area'] ?? 0,
            'm_tables'             => $mTables,
            'm_partsinfo_columns'  => $columns,
    ...
    public function saveAction($requestParams = []) {
        ...
        // --- 修正箇所: Multi Record Confirm の行数強制固定 ---
        $partsType = $requestParams['parts_type'] ?? $_POST['parts_type'] ?? '';
        $inputRows = $requestParams['input_rows'] ?? $_POST['input_rows'] ?? '1';
        if ($partsType === 'Multi Record Confirm') {
            $inputRows = '3';
        }
        ...
        $data = [
            ...
            // --- 修正箇所: 新規フラグの保存処理 ---
            'search_area'       => isset($_POST['search_area']) ? 1 : 0,
            'detail_area'       => isset($_POST['detail_area']) ? 1 : 0
        ];
